...
from flask import Flask, render_template, appcontext_tearing_down, request, redirect, url_for
import sqlite3

from pyexpat.errors import messages
from werkzeug.security import generate_password_hash,  check_password_hash

connection = sqlite3.connect('sqlite.db', check_same_thread=True)
...
app = Flask(__name__)
app.config["SECRET_KEY"] = "f#2#4zD_GF333"
...
def register():
    ...
    if request.method == ("POST"):
        ...
        try:
            ...
            connection.commit()
            print("Р РµРіРёСЃС‚СЂР°С†РёСЏ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ РїСЂРѕС€Р»Р° СѓСЃРїРµС€РЅРѕ")
...
def post(post_id):
    ...
    post_dict = {"id": result[0], "title": result[1], "content": result[2], 
                 "author_id": result[3], "username": result[4]}
    connection.close()
...
def index():
    ...
    for post in reversed(result):
        posts.append({"id": post[0], "title": post[1], "content": post[2],
                      "author.id": post[3], "username": post[4], "likes": post[5]})
